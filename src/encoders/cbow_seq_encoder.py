...
class CBoWEncoder(MaskedSeqEncoder):
    ...
    def make_model(self, is_train: bool=False) -> tf.Tensor:
        with tf.compat.v1.variable_scope("cbow_encoder"):
            ...
            self.seq_tokens_embeddings = tf.pad(tensor=self.seq_tokens_embeddings, paddings=paddings, mode="CONSTANT")
            ...
            # perm dims
            self.seq_tokens_embeddings = tf.transpose(a=self.seq_tokens_embeddings, perm=[1, 0, 2]) # batch size x max seq len x emb dim
    ...
    def token_sums(self, t):
        ...
        return tf.reduce_sum(input_tensor=context, axis=1) # batch size x emb dim
